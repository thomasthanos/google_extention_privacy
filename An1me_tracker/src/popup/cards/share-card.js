// share-card.js — renders a shareable image with your stats/progress drawn onto it.
(function () {
  "use strict";
  ...
  const RENDER_W = OUT_W * 2;
  const RENDER_H = OUT_H * 2;
  ...
  async function render(animeData, index) {
    ...
    const stats = [
      {
        num: String(weekly.episodes),
        lbl: "Episodes",
        accent: "#ff8a5c",
        glow: "rgba(255,138,92,0.18)",
        sparkleColor: "rgba(255,180,140,0.9)",
      },
      {
        num: fmtH(weekly.seconds),
        lbl: "Watch Time",
        accent: "#c084fc",
        glow: "rgba(192,132,252,0.14)",
        sparkleColor: "rgba(220,180,255,0.9)",
      },
      {
        num: `${streak.currentStreak}`,
        lbl: "Day Streak",
        accent: "#fbbf24",
        glow: "rgba(251,191,36,0.14)",
        sparkleColor: "rgba(255,230,140,0.9)",
        icon: "🔥",
      },
    ];
    ...
    const top = StatsEngine.topAnimeInWindow(animeData, 7, 4, window.AnimeTracker?.PopupState?.libraryRevision);
    if (top.length === 0) {
      ctx.fillStyle = "rgba(255,255,255,0.3)";
      ...
      const badgeX = RX + 14 * SCALE,
        badgeY = heroY + 14 * SCALE;
  ...
  async function generateAndOpen(animeData, index) {
    try {
      ...
      if (!dataUrl || dataUrl === "data:,") {
        _toast("Could not generate share card.", "error");
        return;
      }
      ...
      try {
        ...
        try {
          window.PopupLogger?.warn?.("ShareCard", "Could not open viewer, saving instead:", e?.message || e);
        } catch {}
