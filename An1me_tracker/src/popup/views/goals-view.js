// goals-view.js — the Goals screen (daily/weekly/monthly goals, levels).
(function () {
  "use strict";
  ...
  const CHECK_SVG =
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="5 12 10 17 19 7"/></svg>';
  ...
  function renderTrophy(tier) {
    ...
    let extra = "";
    if (rank >= 1)
      extra += '<path class="ach-emblem" d="M12 6.7l.69 1.4 1.55.22-1.12 1.09.27 1.54L12 10.23l-1.38.72.27-1.54-1.12-1.09 1.55-.22z"/>';
    if (rank >= 2) extra += '<path d="M8 1.2l1.5 1.4L12 1l2.5 1.6L16 1.2v2.2H8z"/>';
    if (rank >= 3)
      extra +=
        '<path d="M3 5.6l.5 1.15L4.65 7.3l-1.15.5L3 8.95l-.5-1.15L1.35 7.3 2.5 6.75zM21 5.6l.5 1.15 1.15.55-1.15.5-.5 1.15-.5-1.15-1.15-.5 1.15-.55z"/>';
  ...
  function renderGoalsSection(goals, goalSettings, smartPlan) {
    const rows = Object.entries(GOAL_META)
      .map(([key, meta]) => {
        const goal = goals[key];
        const pct = Math.round((goal.pct || 0) * 100);
        const currentTarget = goalSettings?.[key]?.[meta.field] ?? (meta.field === "targetMinutes" ? 60 : key === "weekly" ? 5 : 20);
        ...
        const smartLabel = smart ? (hasBias ? (biasPct > 0 ? `Tuned +${biasPct}%` : `Tuned ${biasPct}%`) : "Auto") : "";
        const smartLabelClass = hasBias ? "goal-card-chip goal-card-chip--suggested" : "goal-card-chip";
  ...
  function renderRing(pct, opts = {}) {
    ...
    const c = size / 2;
    const center = completed ? `<span class="ach-ring-check">${CHECK_SVG}</span>` : "";
    const inner = showPct
      ? `<span class="ach-ring-pct">${Math.round(clamped * 100)}%</span>`
      : iconHtml
        ? `<span class="ach-ring-icon">${iconHtml}</span>`
        : "";
    return `
  ...
  function buildFamilyStates(badges) {
    ...
    for (const family of ACHIEVEMENT_FAMILIES) {
      ...
      states.push({
        ...
        pct,
        tier,
        statsLabel: formatBadgeProgress(nextStage),
  ...
  function renderFeaturedNextUp(states) {
    const candidate = states.filter((s) => !s.completed && s.pct > 0).sort((a, b) => b.pct - a.pct)[0];
    if (!candidate) return "";
    return `
  ...
  function renderAchievementCard(state) {
    ...
    const tierLabel = state.completed ? `${TIER_LABEL[state.tier] || ""} · Complete` : TIER_LABEL[state.tier] || state.tier;
    return `
  ...
  function openAchievementDetail(familyId) {
    ...
    const groupTitle = (window.AnimeTracker?.BadgeEngine?.GROUP_DEFS || []).find((g) => g.id === state.group)?.title || "";
    ...
    const rows = state.stages
      .map((stage) => {
        ...
        const val = stage.unlocked ? `<span class="ach-check">${CHECK_SVG}</span>` : escapeHtml(formatBadgeProgress(stage));
        return `
  ...
  function render(container, params) {
    ...
    const { index, badges, goals, smartPlan } = params || {};
    const effectiveGoalSettings = params?.effectiveGoalSettings || params?.goalSettings || {};
    if (!Array.isArray(badges) || !goals) {
    ...
    wireInputs(container, { ...params, goalSettings: effectiveGoalSettings });
  }
  ...
  function updateGoalCardInPlace(container, params, key, newTarget) {
    ...
    const goals = GoalEngine.evaluateGoals(provisionalSettings, params.index);
    const goal = goals[key];
    if (!goal) return;
  ...
  function wireInputs(container, params) {
    ...
    steppers.forEach((stepper) => {
      ...
      let saveTimer = null;
      let editVersion = 0;
      let committedSettings = params.goalSettings || {};
      let committedValue = Number(committedSettings?.[key]?.[field]);
      if (!Number.isFinite(committedValue)) committedValue = Number(valueEl?.textContent) || min;

      const persist = async (value, version) => {
        try {
          if (typeof params.onGoalTargetChange !== "function") throw new Error("Goal persistence handler is unavailable");
          const savedSettings = await params.onGoalTargetChange({ key, field, value });
          committedSettings = savedSettings || committedSettings;
          const savedValue = Number(committedSettings?.[key]?.[field]);
          if (Number.isFinite(savedValue)) committedValue = savedValue;
          if (version !== editVersion) return;
          params.goalSettings = committedSettings;
          if (valueEl?.isConnected) {
            valueEl.textContent = String(committedValue);
            updateGoalCardInPlace(container, params, key, committedValue);
          }
        } catch (err) {
          (window.PopupLogger?.warn || console.warn)("GoalsView", "Failed to save goal:", err);
          if (version !== editVersion || !valueEl?.isConnected) return;
          params.goalSettings = committedSettings;
          valueEl.textContent = String(committedValue);
          updateGoalCardInPlace(container, params, key, committedValue);
          if (typeof params.onGoalSaveError === "function") params.onGoalSaveError(err);
        }
      };
      ...
      const apply = (delta) => {
        ...
        params.goalSettings = {
          ...(params.goalSettings || {}),
          [key]: { ...((params.goalSettings || {})[key] || {}), [field]: next },
        };
        updateGoalCardInPlace(container, params, key, next);
        clearTimeout(saveTimer);
        const version = editVersion;
        saveTimer = setTimeout(() => persist(next, version), 400);
      };
      ...
      stepper.querySelectorAll("[data-stepper-action]").forEach((btn) => {
        ...
        const startHold = () => {
          holdTimer = setTimeout(() => {
            repeatTimer = setInterval(() => {
              // A re-render replaces container.innerHTML mid-hold; mouseup never reaches
              // the detached button, so the interval must kill itself.
              if (!btn.isConnected) {
                stopHold();
                return;
              }
        ...
        btn.addEventListener("touchend", (e) => {
          e.preventDefault();
          const repeated = repeatTimer !== null;
          stopHold();
          if (!repeated) apply(dir * step);
        });
