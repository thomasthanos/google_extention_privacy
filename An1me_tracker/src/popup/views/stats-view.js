// stats-view.js — the Stats screen (charts & numbers).
(function () {
  "use strict";
  ...
  function renderActivity(byDay) {
    ...
    const palette = ["rgba(255,255,255,0.06)", "rgba(224,80,30,0.25)", "rgba(224,80,30,0.48)", "rgba(224,80,30,0.72)", "#e0501e"];
    ...
    ["", "M", "", "W", "", "F", ""].forEach((lbl, r) => {
      ...
      const t = svgEl("text", {
        x: "2",
        y: String(LBL_H + r * (CELL + GAP) + CELL - 2),
        class: "hm-lbl",
        "font-size": "8",
      });
      t.textContent = lbl;
    ...
    for (let w = 0; w < WEEKS; w++) {
      for (let d = 0; d < 7; d++) {
        ...
        tt.textContent = bkt ? `${fmtDayKey(dk)}: ${bkt.episodes} ep · ${fmtH(bkt.seconds)}` : `${fmtDayKey(dk)}: no activity`;
        rect.appendChild(tt);
    ...
    return el("div", { class: "hm-wrap" }, [el("div", { class: "hm-svg-wrap" }, [root]), legend]);
  }
  ...
  function renderMonthlyBars(byMonth) {
    ...
    months.forEach((m, i) => {
      ...
      const rect = svgEl("rect", {
        x: String(x),
        y: String(Math.max(pT, y)),
        width: String(bw),
        height: String(Math.max(0, barH)),
        rx: "3",
        ry: "3",
        fill,
        class: "bar-rect",
  ...
  function render(container, animeData) {
    ...
    const index = StatsEngine.buildWatchIndex(animeData, window.AnimeTracker?.PopupState?.libraryRevision);
    ...
    const shareBtn = el(
      ...
      [el("span", { text: "📸" }), el("span", { text: "Share week" })],
    );
    ...
    function section(title, content, cls = "") {
      return el(
        "div",
        { class: `stat-card ${cls}` },
        [title ? el("div", { class: "stat-card-title", text: title }) : null, content].filter(Boolean),
      );
    }
