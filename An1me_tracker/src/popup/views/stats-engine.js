// stats-engine.js — the math behind stats (per day/week/month calculations).
(function () {
  "use strict";
  ...
  function getBucketEpisodes(value) {
    return typeof value === "number" ? value : Number(value?.episodes) || 0;
  }
  ...
  function simulateDaysToFinish(remainingEpisodes, baseRate, weekdayFactors, startDate = new Date()) {
    ...
    const factors = Array.isArray(weekdayFactors) && weekdayFactors.length === 7 ? weekdayFactors : Array(7).fill(1);
  ...
  function buildWatchIndex(animeData, revision = null) {
    ...
    for (const slug in animeData || {}) {
      ...
      if (first && last) {
        ...
        const rate = animeWatchedCount / spanDays;
        ...
        perAnime.set(slug, {
          ...
          watchedCount: animeWatchedCount,
    ...
    const totalSpanDays = overallFirst && overallLast ? Math.max(1, daysBetween(dayKey(overallFirst), dayKey(overallLast)) + 1) : 1;
    const globalActiveCounts = sortedDays.map((key) => Number(byDay.get(key)?.episodes) || 0).filter((value) => value > 0);
  ...
  function predictCompletion(anime, index) {
    ...
    const watchedSet = new Set(anime.episodes.map((ep) => Number(ep?.number)).filter((n) => Number.isFinite(n) && n > 0));
    ...
    const animeWeekdayFactors =
      meta?.weekdayFactors || (firstWatch && mostRecentWatch ? buildWeekdayFactors(byDay, firstWatch, mostRecentWatch) : Array(7).fill(1));
    const animeConsistency = meta?.consistency || 0.35;

    let animeBaseRate =
      spanRate * 0.2 + recent30Rate * 0.16 + recent14Rate * 0.27 + recent7Rate * 0.24 + activeDayRate * 0.08 + recentActiveBurst * 0.05;
    if (recent14Count === 0) {
      animeBaseRate = spanRate * 0.58 + Math.min(activeDayRate, spanRate * 1.5) * 0.24 + recentActiveBurst * 0.18;
    } else if (recent7Count === 0) {
      animeBaseRate = spanRate * 0.3 + recent30Rate * 0.16 + recent14Rate * 0.34 + activeDayRate * 0.12 + recentActiveBurst * 0.08;
    }

    const userBaseRate = userProfile
      ? userProfile.recent7Rate * 0.24 +
        userProfile.recent14Rate * 0.3 +
        userProfile.recent30Rate * 0.2 +
        userProfile.overallRate * 0.16 +
        userProfile.activeDayRate * 0.1
      : spanRate;
    ...
    const hardCap = userProfile ? Math.max(3.5, userProfile.activeDayRate * 1.8, userBaseRate * 1.9) : 3.5;
    ...
    const releaseFloorDays =
      !configuredTarget && anilistStatus === "RELEASING" && total > latestAvailable && latestAvailable > 0
        ? (total - latestAvailable) * 7
        : 0;

    const weekdayFactors = blendWeekdayFactors(userProfile?.weekdayFactors, animeWeekdayFactors, clamp((distinctDays - 2) / 14, 0, 0.6));
    ...
    const rateFloor = Math.max(rate * (0.6 - (animeConsistency - 0.5) * 0.1), 1 / 60);
    const rateCeil = Math.max(rateFloor, rate * (1.22 + (1 - animeConsistency) * 0.2));
    ...
    let confidenceScore =
      ...
      clamp(distinctDays / 10, 0, 1) * 0.2 +
      clamp(recent14Count / 6, 0, 1) * 0.2 +
    ...
    return {
      ...
      confidence,
      model:
        remaining <= 0 && allowSingleEpisodeForecast
          ? "next-drop-pace"
          : releaseFloorDays > 0
            ? "release-aware"
            : configuredTarget
  ...
  function topAnimeInWindow(animeData, days = 7, limit = 5, revision = null) {
    const idx = buildWatchIndex(animeData, revision);
  ...
  function categorizeAnime(animeData) {
    ...
    for (const slug in animeData || {}) {
      ...
      const status = window.AnimeTracker?.StatusService?.getStatus?.(slug, a) || null;
      if (status === "dropped") rows.dropped.push(row);
      else if (status === "on_hold") rows.onHold.push(row);
      else if (status === "completed") rows.completed.push(row);
