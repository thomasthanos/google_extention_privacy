  ...
  function verifyHours(data) {
    ...
    for (const slug in data) {
      ...
      for (const ep of anime.episodes) {
        ...
        if (!bucket) {
          bucket = { episodes: 0, seconds: 0, importEpisodes: 0, importSeconds: 0 };
          byDay.set(key, bucket);
        }
        bucket.episodes++;
        bucket.seconds += dur;
        if (isImport) {
          bucket.importEpisodes++;
          bucket.importSeconds += dur;
        }
      }
    }
    ...
    const topDays = Array.from(byDay.entries())
      ...
      .map(([day, b]) => ({
        ...
        episodes: b.episodes,
        fromImport: b.importEpisodes > 0 ? `${b.importEpisodes} ep / ${Math.round(b.importSeconds / 60)} min` : "—",
      }));
