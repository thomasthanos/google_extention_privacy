// logger.js — shared colored console logging with rate-limiting so repeated
// messages don't flood the devtools console.
(function () {
  "use strict";
  ...
  const PopupLogger = {
    log(tag, ...args) {
      if (!shouldPopupLog("INFO")) return;
      popupStyled(rawLog, tag, args, POPUP_COMPACT);
    },
    once(tag, key, ...args) {
      if (!shouldPopupLog("INFO")) return;
      popupOnce(tag, key, rawLog, args, POPUP_COMPACT);
    },
    throttled(tag, key, intervalMs, ...args) {
      if (!shouldPopupLog("INFO")) return;
      popupThrottled(tag, key, intervalMs, rawLog, args, POPUP_COMPACT);
    },
    debug(tag, ...args) {
      if (!shouldPopupLog("DEBUG")) return;
      popupStyled(rawDebug, tag, args);
    },
    warn(tag, ...args) {
      if (!shouldPopupLog("WARN")) return;
      popupStyled(rawWarn, tag, args);
    },
    error(tag, ...args) {
      popupStyled(rawError, tag, args);
    },
  };
  ...
  const ContentLogger = {
    ...
    styles: {
      prefix:
        "background: linear-gradient(135deg, #ff6b6b, #ff8e53); color: white; padding: 3px 8px; border-radius: 4px; font-weight: 700; font-size: 11px; box-shadow: 0 2px 4px rgba(255,107,107,0.3);",
      badge: (color, bg) =>
        `background: ${bg}; color: ${color}; padding: 2px 6px; border-radius: 4px; font-weight: 600; font-size: 11px; border: 1px solid ${color}33;`,
  ...
  // Debug-gated swallow: makes an otherwise-silent caught error observable when the log level
  // is DEBUG, but stays silent in production (default INFO). Behavior-preserving — never throws.
  function swallow(ctx, err) {
    try {
      PopupLogger.debug("Swallow", `${ctx}: ${(err && err.message) || err}`);
    } catch {}
  }
