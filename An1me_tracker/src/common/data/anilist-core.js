// anilist-core.js — shared AniList API core (GraphQL queries, auth, caching, the
// push schema). Used by both the popup and the background sync job.
(function () {
  "use strict";
  ...
  async function mutateCoordinated(label, keys, operation) {
    if (typeof runBgLibraryTransaction === "function") {
      return runBgLibraryTransaction(keys, operation);
    }

    const coordinator = globalThis.AnimeTracker?.LibraryMutations;
    if (coordinator?.enqueueWithKeys) {
      return coordinator.enqueueWithKeys(label, keys, async ({ snapshot, commit }) => {
        const outcome = await operation(snapshot || {});
        if (outcome?.data) {
          await commit(outcome.data, { sync: false, label });
        }
        return outcome?.result;
      });
    }

    const snapshot = await sget(keys);
    const outcome = await operation(snapshot || {});
  ...
  function paced() {
    const run = _paceTail.then(async () => {
      const now = Date.now();
      const wait = Math.max(REQUEST_GAP_MS - (now - _lastReqAt), _rateLimitedUntil - now);
  ...
  async function gql(query, variables, token) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      if (res.status === 429) {
        ...
        _rateLimitedUntil = Math.max(_rateLimitedUntil, Date.now() + Math.min(waitMs, 65000));
        if (attempt === 0) {
  ...
  function pushStatus(entry) {
    const state = globalThis.AnimeTrackerEntryState?.getResolvedListState?.(entry) || entry.listState || "active";
    if (state === "completed") return "COMPLETED";
    if (state === "dropped") return "DROPPED";
    if (state === "on_hold") return "PAUSED";
    return "CURRENT";
  }
  ...
  function watchDates(entry, status) {
    ...
    const completedIso = status === "COMPLETED" ? maxIso || (hasRealEpisode && entry && entry.completedAt) || null : null;
    return {
  ...
  async function resolveMedia(slug, title, hints = null) {
    ...
    const formatHint = globalThis.AnimeTrackerMediaType?.normalize(hints?.formatHint || hints?.mediaType) || _formatFromSlug(slug);
    const requestedIdentity = _normalizeTitle(_cleanTitleForSearch(title) || title);
    const requestedResolverIdentity = buildResolverIdentityKey(title, formatHint, hints?.totalEpisodes);

    if (cached) {
      if (cached.notFound) {
        const sameVersion = (cached.resolverV || 0) >= RESOLVER_V;
        const sameIdentity = cached.resolverIdentityKey === requestedResolverIdentity;
        if (sameVersion && sameIdentity && now - (cached.cachedAt || 0) < MEDIA_NOTFOUND_TTL) return null;
      } else if (
        cached.mediaId &&
        (cached.resolverV || 0) >= RESOLVER_V &&
        now - (cached.cachedAt || 0) < MEDIA_CACHE_TTL &&
        (["anilistImport", "idMal"].includes(cached.source) || cached.resolverIdentityKey === requestedResolverIdentity)
      ) {
        return cached;
      }
    }
    ...
    try {
      if (Number.isFinite(malIdHint) && malIdHint > 0) {
        try {
          const data = await gql("query($id:Int){Media(idMal:$id,type:ANIME){id episodes format}}", { id: Math.floor(malIdHint) }, null);
          const m = data && data.Media;
          if (m && m.id) {
            result = {
              mediaId: m.id,
              episodes: m.episodes || null,
              format: globalThis.AnimeTrackerMediaType?.normalize(m.format) || m.format || null,
              identityKey: requestedIdentity,
              resolverIdentityKey: requestedResolverIdentity,
              cachedAt: now,
              resolverV: RESOLVER_V,
              source: "idMal",
            };
          }
        } catch (e) {
      ...
      if (!result) {
        ...
        if (bestPick) {
          result = {
            mediaId: bestPick.id,
            episodes: bestPick.episodes || null,
            format: globalThis.AnimeTrackerMediaType?.normalize(bestPick.format) || bestPick.format || null,
            identityKey: requestedIdentity,
            resolverIdentityKey: requestedResolverIdentity,
            cachedAt: now,
            resolverV: RESOLVER_V,
    ...
    const selectedEntry = await mutateCoordinated(`anilist-media:${slug}`, [MEDIA_MAP_KEY], (latestStore) => {
      const latestMap = { ...(latestStore[MEDIA_MAP_KEY] || {}) };
      const latestEntry = latestMap[slug];
      const keepLatestImport = latestEntry?.source === "anilistImport" && resolvedEntry.source !== "anilistImport";
      const keepLatestSuccess =
        resolvedEntry.notFound &&
        latestEntry?.mediaId &&
        Number(latestEntry.cachedAt || 0) >= Number(resolvedEntry.cachedAt || 0);
      if (keepLatestImport || keepLatestSuccess) return { result: latestEntry };
      latestMap[slug] = resolvedEntry;
      return { data: { [MEDIA_MAP_KEY]: latestMap }, result: resolvedEntry };
    });
    return selectedEntry?.notFound ? null : selectedEntry || null;
  }
  ...
  function buildResolverIdentityKey(title, format, totalEpisodes) {
    const titleKey = _normalizeTitle(_cleanTitleForSearch(title) || title);
    const formatKey = globalThis.AnimeTrackerMediaType?.normalize(format) || String(format || "").toUpperCase();
    const total = Number(totalEpisodes);
    return `${titleKey}|${formatKey}|${Number.isFinite(total) && total > 0 ? total : ""}`;
  }
  ...
  async function runPush({ token, maxWork = 1e9, onProgress } = {}) {
    ...
    let pushed = await mutateCoordinated("anilist-push-schema", [PUSHED_KEY, SCHEMA_KEY], (latest) => {
      if (Number(latest[SCHEMA_KEY]) === PUSH_SCHEMA) {
        return { result: latest[PUSHED_KEY] || {} };
      }
      return {
        data: { [PUSHED_KEY]: {}, [SCHEMA_KEY]: PUSH_SCHEMA },
        result: {},
      };
    });
    ...
    const isFreshNotFound = (slug, resolverIdentityKey) => {
      const c = mediaMap[slug];
      if (!c || !c.notFound) return false;
      if ((c.resolverV || 0) < RESOLVER_V) return false;
      if (c.resolverIdentityKey !== resolverIdentityKey) return false;
    ...
    for (const slug of slugs) {
      ...
      if (eps.length > 0 && eps.every((ep) => ep && ep.durationSource === "anilist")) {
        migrationSlugs.push(slug);
      }
    }
    if (migrationSlugs.length > 0) {
      pushed = await mutateCoordinated("anilist-push-lock-migration", [PUSHED_KEY], (latest) => {
        const next = { ...(latest[PUSHED_KEY] || {}) };
        let changed = false;
        for (const slug of migrationSlugs) {
          if (!next[slug] || next[slug].datesLocked != null) continue;
          next[slug] = { ...next[slug], datesLocked: true };
          changed = true;
        }
        return changed
          ? { data: { [PUSHED_KEY]: next }, result: next }
          : { result: next };
      });
    }
    ...
    for (const slug of slugs) {
      ...
      const formatHint = globalThis.AnimeTrackerMediaType?.normalize(entry?.mediaType) || null;
      const resolverIdentityKey = buildResolverIdentityKey(entry?.title || slug, formatHint, totalEpHint);
      const mappedEntry = mediaMap[slug];
      const mappedEntryTrusted =
        mappedEntry?.source === "anilistImport" ||
        mappedEntry?.source === "idMal" ||
        Number(mappedEntry?.resolverV || 0) >= RESOLVER_V;
      const mappedMediaId = mappedEntryTrusted ? Number(mappedEntry?.mediaId) || 0 : 0;
      ...
      if (
        ...
        Number(prev.resolverV || 0) >= RESOLVER_V &&
        prev.resolverIdentityKey === resolverIdentityKey &&
        (!mappedMediaId || Number(prev.mediaId) === mappedMediaId)
      ) {
        done++;
        skipped++;
        if (onProgress) onProgress({ done, total, ok, skipped, failed });
      ...
      if (isFreshNotFound(slug, resolverIdentityKey)) {
        done++;
        failed++;
        if (onProgress) onProgress({ done, total, ok, skipped, failed });
        continue;
      ...
      if (onProgress)
        onProgress({ done, total, ok, skipped, failed, currentSlug: slug, currentTitle: entry?.title || slug, phase: "resolving" });
      ...
      try {
        ...
        if (!media || !media.mediaId) {
          ...
          if (startedAt) {
            varDefs.push("$sa:FuzzyDateInput");
            args.push("startedAt:$sa");
            vars.sa = startedAt;
          }
          if (completedAt) {
            varDefs.push("$ca:FuzzyDateInput");
            args.push("completedAt:$ca");
            vars.ca = completedAt;
          }
          await gql(`mutation(${varDefs.join(",")}){SaveMediaListEntry(${args.join(",")}){id}}`, vars, token);
          const pushEntry = {
            progress,
            status,
            startedAtKey,
          ...
          // Keep the lock while progress hasn't advanced past the imported point — dropping it
          // here would recompute and overwrite the user's historical dates on the next run.
          if (prev && prev.datesLocked && progress <= (prev.progress || 0)) {
            pushEntry.datesLocked = true;
          }
          pushed = await mutateCoordinated(`anilist-pushed:${slug}`, [PUSHED_KEY], (latest) => {
            const next = { ...(latest[PUSHED_KEY] || {}) };
            const latestPrev = next[slug];
            if (latestPrev?.datesLocked && progress <= (latestPrev.progress || 0)) {
              pushEntry.datesLocked = true;
            }
            next[slug] = pushEntry;
            return { data: { [PUSHED_KEY]: next }, result: next };
          });
          ok++;
        }
      } catch (e) {
        ...
        if (isRetryable) retryableFailed++;
        else failed++;
      }
