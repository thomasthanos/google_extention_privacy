...
(function () {
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            #${PANEL_ID}.is-complete {
                border-color: rgba(80, 220, 140, 0.65);
                box-shadow: 0 14px 36px rgba(80, 220, 140, 0.18);
            }
            ...
            #${PANEL_ID} .at-skip-header.dragging { cursor: grabbing; }
            ...
            #${PANEL_ID} .at-skip-close:hover { color: #fff; }
            ...
            #${PANEL_ID} .at-skip-label { font-weight: 600; font-size: 11px; }
            ...
            @media (prefers-reduced-motion: reduce) {
                #${PANEL_ID}, #${PANEL_ID} .at-skip-row { transition: none !important; }
            }
        `;
    ...
    function buildPanelHtml() {
        ...
        return `
            <div class="at-skip-header">
                ...
                <button class="at-skip-close" type="button" aria-label="Hide">×</button>
            </div>
    ...
    async function loadPosition() {
        try {
            ...
            const pos = result[STORAGE_POSITION_KEY];
    ...
    async function savePosition(x, y) {
        try {
            await chrome.storage.local.set({ [STORAGE_POSITION_KEY]: { x, y } });
        } catch {}
    }

    function attachDragHandlers(panel) {
        const header = panel.querySelector('.at-skip-header');
        if (!header) return;

        let dragOriginX = 0, dragOriginY = 0;
        let panelOriginX = 0, panelOriginY = 0;
        let dragging = false;
        ...
        const onMouseMove = (e) => {
            ...
            const maxX = parent.clientWidth - panel.offsetWidth - 8;
            const maxY = parent.clientHeight - panel.offsetHeight - 8;
            const x = Math.max(8, Math.min(maxX, panelOriginX + dx));
            const y = Math.max(8, Math.min(maxY, panelOriginY + dy));
        ...
        header.addEventListener('mousedown', (e) => {
            ...
            if (e.target.closest('.at-skip-close')) return;
            dragging = true;
            header.classList.add('dragging');
            dragOriginX = e.clientX;
            dragOriginY = e.clientY;
            panelOriginX = panel.offsetLeft;
            panelOriginY = panel.offsetTop;
            e.preventDefault();
    ...
    function findPlayerContainer() {
        // an1me.to uses ArtPlayer; class names vary by version.
        // Try the well-known ones first…
        const known = [
            '.art-video-player',
            '.artplayer-app',
            '.art-player',
        ...
        for (const sel of known) {
            ...
            if (el) {
                Logger.info(`Skiptime: container matched ${sel}`);
                return el;
            }
        }
        // …then walk up from the video element looking for a sized ancestor
        // that's a sensible mount point (skip <body>/<html>).
        const v = getVideoElement();
        if (v) {
            let node = v.parentElement;
            while (node && node !== document.body) {
                const cls = node.className || '';
                if (typeof cls === 'string' && /art|player|video/i.test(cls) && node.offsetWidth > 200) {
                    Logger.info('Skiptime: container matched ancestor:', node.className);
                    return node;
                }
                node = node.parentElement;
            }
            // Last fallback: direct parent of the <video>.
            if (v.parentElement) {
                Logger.info('Skiptime: container falling back to video parent:', v.parentElement.className || v.parentElement.tagName);
                return v.parentElement;
            }
        }
        Logger.info('Skiptime: NO player container found, mounting on body');
    ...
    async function mountPanel() {
        ...
        try {
            ...
            // Mount on document.body with fixed positioning. We don't try to
            // host the panel inside the player container any more — it adds
            // failure modes (player container not found, position:static
            // wrapper, z-index stacking) for zero benefit. Fixed positioning
            // is rock-solid: the panel always shows up top-right.
            const host = document.body || document.documentElement;
            if (!host) {
                Logger.info('Skiptime: no document.body yet — retrying in 500ms');
                setTimeout(() => { if (!mounted) mountPanel(); }, 500);
                return;
            }
            ...
            Logger.info('Skiptime: panel appended to', host.tagName);

            // Restore saved position (still works with fixed positioning).
            ...
            // Reset / close.
            panelEl.querySelector('.at-skip-reset')?.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                resetCache();
            });
            panelEl.querySelector('.at-skip-close')?.addEventListener('click', async (e) => {
                e.preventDefault();
                e.stopPropagation();
                // Closing the panel disables the helper for this session AND
                // persists the choice so the popup toggle stays in sync.
                try { await chrome.storage.local.set({ [STORAGE_TOGGLE_KEY]: false }); }
                catch {}
            });

            attachDragHandlers(panelEl);

            // If the video is already there, hook into its metadata for
            // outro-end auto-fill. Otherwise the videoObserver in
            // scheduleMount() will call attachVideoMetadataHooks later.
            if (video) attachVideoMetadataHooks();

            // Keyboard shortcuts (1/2/3/4/0).
            document.addEventListener('keydown', onKeyDown, true);

            // SPA navigation: tear down and re-mount on URL change so we get a
            // fresh cache for the new episode.
            if (!urlObserver) {
                urlObserver = new MutationObserver(() => {
                    if (location.href === lastUrl) return;
                    lastUrl = location.href;
                    Logger.info('Skiptime: URL changed → remount');
                    unmountPanel();
            ...
            await refreshPanelState();
            mounted = true;
            Logger.info('Skiptime helper mounted ✓ (look top-right of page)');
        } catch (err) {
    ...
    function scheduleMount() {
        if (mounted) return;
        // Mount the panel IMMEDIATELY — don't wait for the video element.
        // The panel is fixed-positioned on top of the page and works fine
        // before the player has even initialized. We pick up the video
        // element lazily inside captureTimestamp() / primeOutroEnd().
        Logger.info('Skiptime: scheduleMount → mounting now');
        video = getVideoElement(); // may be null at this point — that's OK
        mountPanel();

        // If the video element wasn't ready yet, watch for it so we can
        // wire up loadedmetadata listeners once it appears.
        if (!video) {
    ...
    async function init() {
        ...
        try {
            ...
            if (enabled) {
                Logger.info('Skiptime: scheduling mount…');
                scheduleMount();
            } else {
