// skiptime.js — Skip Intro/Outro feature (merged skiptime-utils + -styles + -helper):
// stateless helpers + config, the skip-button stylesheet, and the button logic.
(function () {
  "use strict";
  ...
  function getFallbackEpisodeIdentity() {
    try {
      ...
      if (nestedWatchMatch) {
        ...
        const episodeMatch = nestedWatchMatch[2].match(/(?:^|[-_])ep(?:isode)?[-_]?(\d+)/i) || nestedWatchMatch[2].match(/(\d+)/);
...
// skiptime-styles.js — stylesheet for the skip button (depends only on element id constants).
(function () {
  "use strict";
...
// skiptime-helper.js — the Skip Intro / Skip Outro button: main logic.
(function () {
  "use strict";
  ...
  function findSkiptimeOpenButton() {
    ...
    const selectors = ['[data-target="#an1-skip-panel"]', '[aria-controls="an1-skip-panel"]', '[data-bs-target="#an1-skip-panel"]'];
    for (const selector of selectors) {
  ...
  async function ensureSkipPanelOpen() {
    ...
    if (!openBtn) {
      Logger.throttled(`skiptime-open-btn-missing:${getEpisodeIdentity()}`, "WARN", 10000, "Skiptime: Add Skiptime button not found", {
  ...
  function ensureControlsHostVisible(host) {
    ...
    try {
      host.closest(".art-controls")?.style.setProperty("overflow", "visible");
    } catch {}
    try {
      host.closest(".art-bottom")?.style.setProperty("overflow", "visible");
    } catch {}
    try {
    ...
    try {
      ...
      if (closeBtn) {
        ...
        closeBtn.addEventListener("click", async (event) => {
          ...
          event.stopPropagation();
          try {
            await window.AnimeTrackerContent.Storage.set({
              [STORAGE_TOGGLE_KEY]: false,
              playbackSettingsUpdatedAt: new Date().toISOString(),
            });
            await chrome.runtime.sendMessage({ type: "PUSH_STORED_PLAYBACK_SETTINGS" });
          } catch {}
        });
      }
