...
const FILLER_SLUG_SCHEMA_VERSION = 3;
...
async function discoverFillerSlug(an1meSlug, animeTitle, options = {}) {
  ...
  try {
    ...
    if (cached !== undefined) {
      if (cached?.schemaVersion === FILLER_SLUG_SCHEMA_VERSION && typeof cached.slug === "string") {
        fillerSlugCache.set(cacheKey, cached.slug);
        return cached.slug;
      }
      if (cached?.schemaVersion === FILLER_SLUG_SCHEMA_VERSION && cached?.notFound) {
  ...
  if (candidates.length === 0) {
    const notFoundEntry = { notFound: true, schemaVersion: FILLER_SLUG_SCHEMA_VERSION, cachedAt: Date.now() };
    fillerSlugCache.set(cacheKey, null);
    try {
  ...
  const tryCandidate = async (candidate) => {
    ...
    try {
      const resp = await fetch(`https://www.animefillerlist.com/shows/${candidate}`, { method: "HEAD", signal: perCtrl.signal });
  ...
  const candidateResults = await Promise.allSettled(candidates.map(tryCandidate));
  const firstMatch = candidateResults.find((result) => result.status === "fulfilled");
  found = firstMatch?.value || null;
  const errors = candidateResults.filter((result) => result.status === "rejected").map((result) => result.reason);
  allWere404 = errors.length === candidates.length && errors.every((error) => error?.httpStatus === 404);
  groupCtrl.abort();

  if (found) {
    fillerSlugCache.set(cacheKey, found);
    await bgStorageSet({
      [storageKey]: { slug: found, schemaVersion: FILLER_SLUG_SCHEMA_VERSION, cachedAt: Date.now() },
    });
    (typeof dlog === "function" ? dlog : () => {})(`[AnimeTracker] Filler slug discovered: ${an1meSlug} → ${found}`);
    return found;
  }
  ...
  (typeof dlog === "function" ? dlog : () => {})(`[AnimeTracker] No filler data for ${an1meSlug} (tried ${candidates.length} candidates)`);
  return null;
}
...
async function fetchJikanEpisodes(title) {
  try {
    ...
    const anime = (searchData?.data || []).find((candidate) => {
      ...
      return titles.some((candidateTitle) => {
        const normalizedCandidate = normalizeTitle(candidateTitle);
        return normalizedCandidate && normalizedCandidate === expectedTitle;
      });
    });
