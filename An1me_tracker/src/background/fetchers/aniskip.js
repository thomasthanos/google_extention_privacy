// aniskip.js — resolves an1me slug → MAL id (via Jikan) and fetches outro start
// times from AniSkip for the Skip-Outro button. Results are cached in two bundled
// storage maps with TTLs; includes a one-time migration from legacy per-key caches.
...
// Write-through: a deferred (setTimeout) flush is lost when the MV3 service worker is torn
// down, forcing the same Jikan/AniSkip lookups to be re-fetched. Writes are rare (one per
// fetch), so persisting immediately is cheap.
function scheduleAniSkipBundleFlush() {
  if (!_aniSkipBundle) return;
  bgStorageSet({ [ANISKIP_BUNDLE_KEY]: _aniSkipBundle }).catch(() => {});
}

function scheduleSlugMalBundleFlush() {
  if (!_slugMalBundle) return;
...
async function fetchAniSkipOutroStart(slug, title, episodeNumber, episodeLength) {
  ...
  try {
    ...
    const lengthParam = Number.isFinite(episodeLength) && episodeLength > 0 ? `&episodeLength=${Math.round(episodeLength)}` : "";
    let res;
    try {
      res = await fetch(`https://api.aniskip.com/v2/skip-times/${malId}/${episodeNumber}?types[]=ed${lengthParam}`, {
        signal: ctrl.signal,
      });
    } finally {
      clearTimeout(timer);
...
async function migratePerKeyCachesOnce() {
  try {
    ...
    if (toDelete.length > 0) {
      try {
        await bgStorageRemove(toDelete);
      } catch {}
      (typeof dlog === "function" ? dlog : () => {})(`[BG] Migrated ${toDelete.length} per-key cache entries → 2 bundles`);
    }
  } catch (e) {
